...
namespace Embedly
{
    ...
    public class Client
    {
        ...
        public Client(string key) : this(key, new TimeSpan(0, 1, 0), new TimeSpan(0, 0, 0), null) {}
        ...
        public Client(string key, TimeSpan requestTimeout, TimeSpan bufferTimeout, IResponseCache cache)
        {
            if (string.IsNullOrEmpty(key))
                throw new ArgumentException("Embedly account key cannot be empty", nameof(key));
        ...
        public Provider GetProvider(Uri url)
        {
            ...
            Provider result = Service.Instance.Providers.FirstOrDefault(p => p.IsMatch(url));
